...
def apply_tfidf(train_data:pd.DataFrame,max_features:int,ngram_range:tuple) -> tuple:
    """Apply TF-IDF to ngram to the data."""
    try:
...
def train_lgbm(X_train:np.ndarray,y_train:np.ndarray,learning_rate:float,max_depth:int,n_estimators:int) -> lgb.LGBMClassifier:
    ...
    try:
        ...
        logger.error('Error during the LightGBM model training')
        raise
def save_model(model,file_path:str)->None:
    ...
    try:
        ...
        logger.debug('Error in saving the model %s',e)
        raise
def main():
    try:
        # Get rooot directory and resolve the path for params.yaml
